...
### Fixed
- Se movió `typescript` a `dependencies` en tiempo de ejecución para asegurar la ejecución independiente mediante `npx` e instalaciones globales sin errores de módulos no encontrados.

### Added
- Flujo automatizado de escaneo de código SARIF en GitHub Actions (`.github/workflows/sarif.yml`).
- Archivo oficial de licencia `LICENSE` (MIT).
- Documentación bilingüe (`README.es.md` y `CHANGELOG.es.md`).
